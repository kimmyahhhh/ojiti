    ...
    public function SubmitOC($data){
        try {
            ...
            foreach ($entries as $entry) {
                ...
                $stmt = $this->conn->prepare("INSERT INTO ".$table." (OCNo, NameTO, NameFROM, Quantity, SRP, SupplierSI, SerialNo, Product, Supplier, Category, Type, Branch, Vat, VatSales, Warranty, DatePrepared, User) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                if (!$stmt) throw new Exception("Prepare failed (insertOC): " . $this->conn->error);
                
                $stmt->bind_param('sssssssssssssssss', 
                    $formattedOCNo, $recipient, $sender, $qty, $srp, $siNo, $serialNo, $productName, 
                    $supplier, $category, $type, $branch, $vat, $vatSales, $warranty, $datePrepared, $user);

...
class AcashInfoProcess extends Database
{
    ...
    public function LoadMaintenanceData($data) {
        $this->CheckTables();
        $rows = [];
        // Fetch only records that have both email and branch set
        $sql = "SELECT email, branch_name, full_name FROM tbl_master_identities WHERE email IS NOT NULL AND email != '' AND branch_name IS NOT NULL AND branch_name != '' ORDER BY id DESC";
        $result = $this->conn->query($sql);
        if ($result) {
            while ($row = $result->fetch_assoc()) {
                $rows[] = $row;

...
class Reports extends Database 
{
    ...
    public function PrintTransmittalByNo($transNo, $templatePath = null){
        ...
        if (!$templatePath) {
            ...
            $templatePath = 'C:\xampp\htdocs\isyn-app-v3\template\resibo.jpg';
            ...
            try {
                if ($logoExt === 'png' && !$supportsPngAlpha) {
                    ...
                    if (file_exists($logoJpg)) {
                        ...
                        error_log('Logo PNG with alpha not supported; using JPG fallback: '.$logoJpg);
                    } else {
                        error_log('Logo PNG requires GD/Imagick. No JPG fallback found. Skipping logo.');
                    }
                } else {
                    $pdf->Image($logoPath, $logoX, $logoY, $logoWidth, $logoHeight, strtoupper($logoExt) === 'PNG' ? 'PNG' : '', '', '', false, 300, '', false, false, 0);
                }
        ...
        $content = '
            ...
            <table border="1" cellpadding="3" cellspacing="0" style="border-collapse:collapse; width:100%; font-size:'.$fontSizeItems.';">
                ...
                <tr>
                    <td colspan="3" style="text-align:center;">Pls. write special instructions below</td>
                </tr>
            </table>

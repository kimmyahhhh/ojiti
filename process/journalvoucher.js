    ...
    $.ajax({
        ...
        success:function(response){
            ...
            $.each(response.NATUREADJUSTMENT,function(key,value){
                let natureVal = value["nature"] || value["Nature"];
                if(natureVal){
                    $("#NatureAdjustment").append(`<option value="${natureVal}">${natureVal}</option>`);
                }
            });

            $.each(response.FUNDS,function(key,value){
                let fundVal = value["Fund"] || value["fund"] || value["FUND"];
                if(fundVal){
                    $("#Fund").append(`<option value="${fundVal}">${fundVal}</option>`);
                }
            });
